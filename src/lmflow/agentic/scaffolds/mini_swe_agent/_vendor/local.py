"""Vendored mini-swe-agent local environment semantics.

LMFlow subclasses this implementation to replace subprocess execution while
retaining upstream submission detection and template/serialization behavior.
"""
...
from lmflow.agentic.scaffolds.mini_swe_agent._vendor.exceptions import Submitted
...
class LocalEnvironment:
    def __init__(self, *, config_class: type = LocalEnvironmentConfig, **kwargs):
        """Execute Bash commands directly on the local machine."""
        self.config = config_class(**kwargs)

    def execute(self, action: dict, cwd: str = "", *, timeout: int | None = None) -> dict[str, Any]:
        """Execute a command in the local environment and return the result."""
    ...
    def _check_finished(self, output: dict):
        """Raise ``Submitted`` if the output indicates task completion."""
